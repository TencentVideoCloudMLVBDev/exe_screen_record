	...
	if (message == WM_COPYDATA)
	{
		...
		case ScreenRecordStart:
		{
			if (!strcmp(message, "RecordStart"))
			{
				...
				m_pusher.setScreenCaptureParam(m_hChooseHwnd, captureRect, true);
...
int Application::run(int &argc, char **argv)
{
	...
	do
	{
		...
		ScreenRecordType recordType = RecordScreenNone;
		if (root.isMember("recordType"))
		{
			//0.后台录制-窗口 1.本地-暂未提供
			recordType = (ScreenRecordType)root["recordType"].asInt();
		}

		if (recordType != RecordScreenToServer)
		{
			LINFO(L"now only support recordtype = RecordScreenToServer");
			break;
		}
		...
		if (recordType == RecordScreenToServer)
		{
			size_t index = recordUrl.find("record");  // 忽略大小写
			if (std::string::npos == index)
			{
			...
			m_pusher.startPush(recordUrl.c_str());
